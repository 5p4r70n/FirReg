...
contract CaseReg{
    ...
    function AddPetition (string memory _Prathy,string memory _Vaadi,string memory _Tittle,CType _Casetyp,string memory _WriteUp) public {
     Pgno++; //page no increament
     MPetition[Pgno]= Petition(_Prathy,_Vaadi,_Tittle,_Casetyp,_WriteUp); //pushing inputs to the structure
    }

    function getPg() public view returns(uint _Pgno) {
        return _Pgno = Pgno;
